...
# 자가치유 — 빈 디렉토리면 마켓 캐시에서 빌트인 페르소나 자동 보충
...
if [ ! -d "$PERSONAS_DIR" ] || [ -z "$(ls -A "$PERSONAS_DIR" 2>/dev/null)" ]; then
  [ "$LANGUAGE" = "ko" ] && echo "❌ 페르소나 프리셋을 찾지 못했습니다. /crosstalk:install 또는 /crosstalk:install --presets-only 실행." || echo "❌ No persona presets found. Run /crosstalk:install or /crosstalk:install --presets-only."
  exit 1
fi
